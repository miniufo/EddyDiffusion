...
public class EddySpeedPDFByGM{
	...
	static void postProcess(List<? extends Particle> ls){
		...
		EulerianStatistics estat=new EulerianStatistics(ls,DiagnosisFactory.DFQuater.getDataDescriptor(),false);
		
		estat.normalizeByGM(new float[]{1,2},1f/365f);
		...
		dataToFile(path,"0.25",new BinningStatistics(template).binningCount(ls).getData()[0][0],dataU,dataV,posiX,posiY);
	}
	...
	static void smoothData(List<? extends Particle> ls){
		...
		if(p.getTCount()>=9){
			...
			FilterModel.runningMean(uvelOrig,uvelSmth,9);
			FilterModel.runningMean(vvelOrig,vvelSmth,9);
