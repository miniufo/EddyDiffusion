...
public final class IdealizedDiffusivity{
	...
	static void cLagrangianStatistics(Region2D region,int tRad,String tag,Method m){
		...
		lstat.cStatisticsByDavisTheory (cond,tRad).toFile(path+"Diff/Lstat"+tag+m+"1.txt");
		lstat.cStatisticsByTaylorTheory(cond,tRad).toFile(path+"Diff/Lstat"+tag+m+"2.txt");
	}
}
